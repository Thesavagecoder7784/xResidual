...
## 32. The most valuable goal is the one nobody's watching: garbage time and the goal-difference cliff

Crossing the goal-difference jeopardy of #27 with the match-leverage idea of #9 turns up a sharp, counterintuitive number. In the 40k conditioned sims, a team that finishes third reaches the Round of 32 **70.6% of the time on −1 goal difference but 98.6% on level GD** — so a single goal dragging it from −1 to even is worth **+28 percentage points** of advancement. The curve is a cliff: flat-and-low below −2, a steep climb from −2 to 0 (each goal worth 17–28pp), then flat-and-high above 0. (`garbage_time`.)

**The trader's read.** Because the cut is a GD tiebreak (#27), the highest-leverage *action* in the group stage isn't winning a marquee match — it's the goal margin in games whose result is already settled. A bubble team 3–0 up should keep scoring, and a team 3–0 down should keep defending, because that fourth goal (or that consolation conceded) can swing its qualification by more than the *outcome* of a different fixture. For an advancement trader the so-what is concrete: in dead-rubber-looking blowouts involving a bubble team, the live variable is the *scoreline running up*, not the result — the leverage hides in garbage time. Caveat: it's the model's conditional curve (large n, but one model on one field), and the effect concentrates entirely at the −1→0 hinge — above level GD a goal is worthless, below −2 nearly so.
