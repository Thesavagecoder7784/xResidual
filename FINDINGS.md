...
A living log. Each entry states what the data shows, then *what it would mean to someone trading or making markets on it* — because the point isn't the chart, it's the decision the chart implies. Methodology in [METHODOLOGY.md](METHODOLOGY.md); numbers regenerate from `scripts/run_analysis.py`.
...
## 3. The two prediction markets agree to ~0.14pp — law of one price holds

Median cross-venue divergence (Kalshi vs Polymarket, de-vigged) on the title race is ~0.14pp; the largest standing gap is England (~1.3pp).
...
beat the market; it's the independent yardstick that lets us *measure surprise*. When
...
is already *lower* (2.48) than at sea level (2.64), and when we regressed total goals on
...
old prior. So we removed the altitude-totals factor from the model entirely.

**The trader's read.** This is the model being disciplined, not the market being wrong:
we had a folk-physics prior, tested it on real data, and it didn't hold, so it's gone.
...
applies only to Mexico at home, and we leave it out rather than widen the host edge.)
...
Our pure Elo/Skellam simulation gives **Spain ~28% to win the title; the market and Opta
have it ~16%** (Argentina ~19% vs ~10%). We chased the cause — it isn't host, altitude,
...
from ~4.7pp to ~0.7pp**, landing the model on the market and Opta at once. (We lack
...
turned out to be **squad value specifically** — fixable, not mysterious — and blending it
...
Polymarket and Kalshi agree to **~0.17pp on average** across the field — the "5–8¢ gap"
...
Betfair Exchange (the sharpest soccer market we log), **Polymarket sits marginally
...
> Kalshi and Polymarket price the World Cup winner within ~0.14pp of each other. The
...
> the two crowds agree on every title price to ~0.17pp. The durable difference is *cost*
