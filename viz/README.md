...
| `garbage_time` | **RETRACTED Jun 22** — the headline "+35pp" was the conditional `P(adv\|3rd,GD)`, which is points-confounded (double-counts the points high-GD teams tend to have). Superseded by `points_first`, the points-controlled **+12pp** version. Do not re-cite the +35pp |
...
| `dead_rubbers` | **NEW** — 18 of the biggest teams have already clinched, so their final games are dead rubbers; the real win-and-in drama is in obscure matchups (Bosnia-Qatar swings 99pp). The format manufactured stakeless games among the giants (Csató-Gyimesi: ~92% for a top seed) |
| `tiebreak_cascade` | **NEW** — a yellow card can end your World Cup: the last R32 spot is level on points 72% of sims, cascading points → GD → goals → **discipline**. Precedent: Senegal 2018, out on 6 yellows to 4 |
...
| `cross_group_butterfly` | **NEW** — your fate is decided by games you're not playing: the 8 best-thirds are picked jointly across all 12 groups, so a Group D result swings a Group G third's advance odds **69%→47%** (a 22pp swing from a match it isn't in). Only a joint sim sees it |
