...
**And it mostly isn't tradeable.** At the goal the order book vanishes — spread ~8× wider, best-price depth under 1% of normal — so across 405 goal-shock observations in 66 matches (each goal moves both of a match's contracts, so roughly twice the count of distinct goals) the median 12.0-cent stale-quote gap a reactor would lift carries a median +10.8c net on paper, but once gated on what's actually resting in the book **the median match yields no harvestable goal at all**. Harvestable goals do exist — across the full 66-match ledger the goal-weighted rate is ~9%, clustered in 21 of those 66 matches — but they're a minority you can't identify in advance, because the depth is gone at the instant the signal fires. That collapse-and-refill is the market-maker pulling quotes against toxic, information-motivated flow: **adverse selection observed in real time**, and the reason the lead is *mostly information, not alpha*. Finding an edge and correctly measuring how little of it survives is the point.
