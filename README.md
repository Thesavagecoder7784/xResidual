...
The crown jewel is a microstructure result, written up as a short **[desk note (PDF)](writeups/price_discovery_note.pdf)**: from tick-level order-book tapes of both venues, **Polymarket discovers the price of a goal first** — a ~600 ms reaction lead across 84 matches (leading 59 of 61 on the information-share decomposition) and a **~80% Gonzalo–Granger information share** (computed on mids, which sidesteps the documented ~59% trade-direction-classification problem), and it's *concentrated at the news* — ~86% of price discovery in goal windows vs ~53% in calm play.

**And it mostly isn't tradeable.** At the goal the order book vanishes — spread ~8× wider, best-price depth under 1% of normal — so the ~12-cent stale-quote gap a reactor would lift nets +10.8c on paper but **essentially 0% of goals clear a tradeable-depth bar at size once gated on what's actually resting in the book — and 0% in the deepest-liquidity knockouts, where you'd put real size** (across all leads, the median edge is under the taker fee, so ~85% are net-negative anyway). That collapse-and-refill is the market-maker pulling quotes against toxic, information-motivated flow: **adverse selection observed in real time**, and the reason the lead is *mostly information, not alpha*. Finding an edge and correctly measuring how little of it survives is the point.
