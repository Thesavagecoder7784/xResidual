...
## The headline finding — a real lead a market-maker mostly can't trade

The crown jewel is a microstructure result, written up as a short **[desk note (PDF)](writeups/price_discovery_note.pdf)**: from tick-level order-book tapes of both venues, **Polymarket discovers the price of a goal first** — a ~600 ms reaction lead across 86 matches (leading 61 of 63 on the information-share decomposition) and a **~81% Gonzalo–Granger information share** (computed on mids, which sidesteps the documented ~59% trade-direction-classification problem), and it's *concentrated at the news* — ~86% of price discovery in goal windows vs ~53% in calm play.

**And it mostly isn't tradeable.** At the goal the order book vanishes — spread ~8× wider, best-price depth under 1% of normal — so across 405 goal-shock observations in 66 matches (each goal moves both of a match's contracts, so roughly twice the count of distinct goals) the median 12.0-cent stale-quote gap a reactor would lift carries a median +10.8c net on paper, but once gated on what's actually resting in the book **the median match yields no harvestable goal at all**. Harvestable goals do exist — on the 21-match subset still reconstructible from the archive the goal-weighted rate is ~11%, clustered in 6 of those 21 matches — but they're a minority you can't identify in advance, because the depth is gone at the instant the signal fires. That collapse-and-refill is the market-maker pulling quotes against toxic, information-motivated flow: **adverse selection observed in real time**, and the reason the lead is *mostly information, not alpha*. Finding an edge and correctly measuring how little of it survives is the point.

It's deliberately falsifiable: the project's forecasts were **pre-registered before kickoff and were graded publicly on July 19**, hits and misses (6 pass, 2 fail, 3 inconclusive) — a track record that can't be edited after the fact ([PREREGISTRATION.md](PREREGISTRATION.md)).
...
Running notes, each framed by the decision it implies, live in [FINDINGS.md](FINDINGS.md). These are descriptive reads — the survivors of a wider exploratory sweep, with the nulls and one retraction kept in rather than buried — and they are deliberately kept distinct from the binding, pre-committed tests below, which are the multiple-comparison-safe scorecard graded on July 19. A few were in before kickoff: Polymarket quoted about 27× Kalshi's depth at the same spread (a gap that compressed as the field narrowed), every title favorite is sell-heavy, the favorite–longshot bias hides in the 1¢ tick, and the "altitude means more goals" idea doesn't survive a look at the data. One pre-tournament scan ran pro-market: a systematic sweep of 238 contracts across five layers finds the market is hard to beat wherever there's liquidity (and where we disagreed, an independent bookmaker sided with the market — it was usually us); chasing that down surfaced a real *model* bug, not a market one — Elo inflates weak, regionally-isolated confederations, now corrected with a validated empirical-Bayes shrinkage.

Since kickoff the live reads have started to land. The cross-venue price-discovery work has matured into the project's flagship: across 86 captured matches Polymarket leads the repricing on goals about 72% of the time (281 vs 111 of 392 decisive events, median +600 ms), and — hardened against the clustering of events within matches — **57 of 66 matches lean Polymarket** (sign-test p ≈ 1.2×10⁻⁹; match-resampling CI [67%, 76%], design effect just 1.13). The information-share decomposition (Hasbrouck 1995; Gonzalo–Granger) puts its share of price discovery near 81% (per-match median, bootstrap CI 77–87%; Polymarket leads 61 of 63 cointegrated matches, p ≈ 4.4×10⁻¹⁶) — computed on order-book mids, so it survives the trade-direction-classification problem that limits most venue comparisons. Two format-native reads stand out: the expanded 48-team field paired a high scoring rate with an unusually high share of draws — the group stage finished at **2.99 goals/game and 27.8% drawn** (unaffected by extra time, so convention-free), and the full 104 matches at **2.96 goals/game and 23.1% drawn counting extra time**, or 2.88 and 27.9% on regulation-time scores — the knockout ties settled in extra time are what move it, so the convention has to be stated. An early-tournament read of 3.09 through 33 games, which is the figure the frozen group-stage card carries, ran hotter than the tournament finished — and the group-stage jeopardy has migrated to goal difference (the last team into the Round of 32 and the first one out finish level on points 72% of the time in the simulation). And an in-play win-probability model, run against the live tape, finds the market under-reacts to a goal by a few points before it settles. Where a tentative pre-tournament edge (mismatch draws) regressed once more games landed, it's marked retracted — hits and misses both.

Before kickoff I locked eleven falsifiable predictions about how these markets would behave — calibration, law of one price, cross-venue lead-lag, sigma-sanity, and the in-play heat test — each with its exact grading rule, in [PREREGISTRATION.md](PREREGISTRATION.md). They were graded in public on July 19: **6 pass, 2 fail, 3 inconclusive**, hits and misses both.
...
- Retrospective, "[The 2026 World Cup, in residuals](writeups/retrospective.md)" — *complete* (post-tournament long-form: the chalk held, my model's errors were almost all draws, and the market had already priced them)
...
- **Microstructure.** Order-book depth and spread, order-book imbalance (do the favorites sit bid or offered?), cross-venue convergence, lead–lag price discovery (which venue moves first on a goal), the Hasbrouck / Gonzalo–Granger information share, order-flow imbalance as the within-venue mechanism (Cont–Kukanov–Stoikov, book-derived so it's immune to the trade-direction problem), and bookmaker dispersion. Reads: at the group stage Polymarket quoted ~27× Kalshi's depth at the same ~0.1¢ spread (compressing toward ~4× by the final four as the field narrowed), every title favorite is sell-heavy (OBI ≈ 0.2), and across the captured match pool Polymarket leads price discovery — about 72% of goal repricings (57 of 66 matches) and an information share near 80% (61 of 63 matches).
...
- `deploy/` — through the tournament, collection ran 24/7 on an **always-on Azure VM** (systemd timers; setup + runbook here), so no match was ever lost to a sleeping laptop, while the laptop kept analysis / rendering / posting and pulled the data down with `make pull`. *Collection ended with the final on July 19 and the VM has since been decommissioned; `deploy/` is kept as the record of how the capture pipeline ran.*
...
- **The market layer came from a live logger you run.** `logger/` records Kalshi / Polymarket / Odds API prices to `logger/data/` (git-ignored, since those feeds aren't mine to redistribute). Through the tournament I ran it 24/7 on an always-on Azure VM (`deploy/`) and pulled the data down with `make pull`, so collection never depended on my laptop being awake. The generated `viz/market/` data and PNGs are committed so the cards render as-is; regenerating them yourself means running the logger with your own API keys (`cp logger/config.example.json logger/config.json`) — though the 2026 World Cup itself is of course no longer streaming, so what the logger can reproduce now is the mechanism, not this dataset.
