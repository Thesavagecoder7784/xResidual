...
The crown jewel is a microstructure result, written up as a short **[desk note (PDF)](writeups/price_discovery_note.pdf)**: from tick-level order-book tapes of both venues, **Polymarket discovers the price of a goal first** — a ~400 ms reaction lead across 57 matches (leading 41 of 42 on the information-share decomposition) and a **~78% Gonzalo–Granger information share** (computed on mids, which sidesteps the documented ~59% trade-direction-classification problem), and it's *concentrated at the news* — ~86% of price discovery in goal windows vs ~53% in calm play.
